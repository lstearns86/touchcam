...
namespace HandSightLibrary.ImageProcessing
{
    public class Localization
    {
        ...
        public Dictionary<string, List<ImageTemplate>> samples = new Dictionary<string, List<ImageTemplate>>();
        Classifier groupClassifier = null, groupClassifierSecondary = null;
        Classifier sensorFusionClassifier = null;
        public Dictionary<string, string> coarseLocations = new Dictionary<string,string>();
        public Dictionary<string, HashSet<string>> groups = new Dictionary<string, HashSet<string>>();
        Dictionary<string, List<Tuple<string, int>>> trainInfoByGroup = new Dictionary<string, List<Tuple<string, int>>>();
        Dictionary<string, Classifier> bfClassifierByGroup = new Dictionary<string, Classifier>();
        Dictionary<string, Classifier> classifierByGroup = new Dictionary<string, Classifier>();
        Dictionary<string, Classifier> secondaryClassifierByGroup = new Dictionary<string, Classifier>();
        Dictionary<string, Classifier> sensorFusionClassifierByGroup = new Dictionary<string, Classifier>();

        private Dictionary<string, bool> needsRetraining = new Dictionary<string, bool>();
        ...
        public void Load(string name)
        {
            ...
            foreach(string filename in Directory.GetFiles(dir, "*.png"))
            {
                ...
                template["coarse"] = group;
                template["fine"] = region;
        ...
        public void AddTrainingExample(ImageTemplate template, string group, string region)
        {
            ...
            if (!needsRetraining.ContainsKey(group)) needsRetraining.Add(group, false);
            needsRetraining[group] = true;
        }

        public void RemoveTrainingExample(ImageTemplate template)
        {
            string group = "", region = "";
        ...
        public void Train(bool enableCamera = true, bool enableSecondary = false, bool forceCompleteRetraining = true)
        {
            ...
            foreach (string key in needsRetraining.Keys) if (needsRetraining[key]) coarseNeedsRetraining = true;
            ...
            foreach (string groupName in groups.Keys)
            {
                ...
                if (!forceCompleteRetraining && !needsRetraining[groupName]) continue;
        ...
        public string PredictCoarseLocation(ImageTemplate query, bool enableCamera = true, bool enableSecondary = false)
        {
        ...
        public string PredictCoarseLocation(ImageTemplate query, out Dictionary<string, float> probabilities, bool enableCamera = true, bool enableSecondary = false)
        {
            ...
            probabilities = probabilitiesTexture;
            if (enableSecondary)
            {
        ...
        public string PredictFineLocation(ImageTemplate query, bool enableCamera = true, bool enableGeometricVerification = true, bool enableSecondary = false, params string[] predictedGroup) { bool temp; Dictionary<string, float> probabilities; return PredictFineLocation(query, out temp, out probabilities, enableCamera, enableGeometricVerification, enableSecondary, predictedGroup); }
        public string PredictFineLocation(ImageTemplate query, out bool foundFeatureMatch, out Dictionary<string, float> probabilities, bool enableCamera = true, bool enableGeometricVerification = true, bool enableSecondary = false, params string[] predictedGroup)
        {
            ...
            Classifier classifier = enableCamera ? (groups.ContainsKey(predictedGroup[0]) ? classifierByGroup[predictedGroup[0]] : null) : null;
            string predictedRegionTexture = classifier != null ? classifier.Predict(query.TextureMatrixRow, out probabilities) : "";
            string predictedRegion = predictedRegionTexture;
            foundFeatureMatch = false;
            if (probabilities.Count == 0) probabilities.Add(predictedRegion, 1);
        ...
        double homographyConstraint1 = 4;
        double homographyConstraint2 = 0.001;
        bool IsGoodHomography(Matrix<double> H)
        {
        ...
        bool AreGoodInliers(VectorOfKeyPoint f1, VectorOfVectorOfDMatch matches, Mat mask)
        {
        ...
        void EvaluateMatch(string className, ImageTemplate template)
        {
