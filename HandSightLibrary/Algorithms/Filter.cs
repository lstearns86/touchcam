...
namespace HandSightLibrary
{
    public abstract class Filter
    {
        ...
        protected Matrix<double> GradientDescent(double a_x, double a_y, double a_z, double m_x, double m_y, double m_z, double mu, Matrix<double> qObserv)
        {
            int i = 0;
            double q1, q2, q3, q4;
            Matrix<double> f_obb = new Matrix<double>(6, 1);
            Matrix<double> Jacobian = new Matrix<double>(6, 4);
            Matrix<double> Df = new Matrix<double>(4, 1);
            double norm;
            double bx, bz, by;
            ...
            while (i < 10)
            {
                ...
                Matrix<double> quaternion = new MyQuaternion(q1, q2, q3, q4).getQuaternionAsVector();
                Matrix<double> bRif = magneticCompensation(new MyQuaternion(q1, q2, q3, q4), m_x, m_y, m_z);
                bx = bRif[0, 0];
                by = bRif[1, 0];
                bz = bRif[2, 0];
                ...
                Df = Jacobian.Transpose() * f_obb;
                norm = Math.Sqrt(Df[0, 0] * Df[0, 0] + Df[1, 0] * Df[1, 0] + Df[2, 0] * Df[2, 0] + Df[3, 0] * Df[3, 0]);
                Df = Df.Mul(1.0 / norm);// MatrixLibrary.Matrix.ScalarDivide(norm, Df);
                ...
                result = result.Mul(1.0 / norm);// MatrixLibrary.Matrix.ScalarDivide(norm, result);
        ...
        protected Matrix<double> gaussNewtonMethod(double a_x, double a_y, double a_z, double m_x, double m_y, double m_z, Matrix<double> qObserv)
        {
            double norm;
            Matrix<double> n = new Matrix<double>(4, 1);
            Matrix<double> bRif = new Matrix<double>(3, 1);
            Matrix<double> jacobian = new Matrix<double>(6, 4);
            Matrix<double> R = new Matrix<double>(6, 6);
            Matrix<double> y_e = new Matrix<double>(6, 1);
            Matrix<double> y_b = new Matrix<double>(6, 1);
            ...
            Matrix<double> n_k = new Matrix<double>(4, 1);
            n_k[0, 0] = a;
            n_k[1, 0] = b;
            n_k[2, 0] = c;
            n_k[3, 0] = d;
            ...
            while (i < 3)
            {
                ...
                Matrix<double> temp = jacobian.Transpose() * jacobian;
                CvInvoke.Invert(temp, temp, Emgu.CV.CvEnum.DecompMethod.Svd);
                n = n_k - temp * jacobian.Transpose() * (y_e - R * y_b);
        ...
        protected Matrix<double> magneticCompensation(MyQuaternion q, double m_x, double m_y, double m_z)
        {

            Matrix<double> h = new Matrix<double>(4, 1);
            Matrix<double> temp;
            //compute the direction of the magnetic field
            Matrix<double> quaternion = q.getQuaternionAsVector();
            Matrix<double> quaternion_conjugate = q.getConjugate();
        ...
        public abstract Matrix<double> getFilteredQuaternions();
        public abstract Matrix<double> getFilteredAngles();
